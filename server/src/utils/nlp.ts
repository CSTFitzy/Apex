...
function toDecimal(value: number, hemisphere?: string): number {
  if (hemisphere && /[SsWw]/.test(hemisphere)) return -Math.abs(value);
  return value;
}

function extractCoordinates(text: string): ExtractedCoordinate[] {
  const coords: ExtractedCoordinate[] = [];
...
export function extractDocumentIntelligence(text: string): DocumentExtraction {
