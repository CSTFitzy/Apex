...
const OPEN_METEO_FORECAST_URL =
  process.env.OPEN_METEO_API || 'https://api.open-meteo.com/v1/forecast';
const OPEN_METEO_ARCHIVE_URL =
  process.env.OPEN_METEO_ARCHIVE_API || 'https://archive-api.open-meteo.com/v1/archive';
...
function assessOperationalImpact(current: Record<string, number>): {
  ...
  if (severity >= 6) level = 'SEVERE';
  else if (severity >= 3) level = 'HIGH';
  else if (severity >= 1) level = 'MODERATE';
