...
import { doctrineProfiles, matchDoctrineProfiles } from '../data/doctrine.js';
const router = Router();
function assessThreatLevel(profiles, friendlyForces) {
    const rationale = [];
    let score = 0;
...
function generateCounterPlan(profiles, friendlyForces, terrainSummary) {
    ...
    if (terrainSummary) {
        recommendedActions.add(`Incorporate terrain analysis findings into the plan: ${terrainSummary}`);
    }
    const enemyTacticsText = profiles
        .map((p) => `${p.name} is likely to: ${p.tactics.join('; ')}.`)
        .join(' ');
    const friendlySummary = friendlyForces.length > 0
        ? friendlyForces
            .map((f) => `${f.name} (${f.composition}${f.strength ? `, ~${f.strength} personnel` : ''})`)
            .join(', ')
        : 'friendly forces (composition not specified)';
    const narrative = `Based on the identified enemy composition, the enemy is expected to employ the following doctrine: ` +
 ...
 */
router.post('/counter-plan', (req, res) => {
    try {
        const body = req.body;
