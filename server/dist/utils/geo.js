...
export function destinationPoint(start, bearingDeg, distanceM) {
    ...
    const lon2 = lon1 +
        Math.atan2(Math.sin(brng) * Math.sin(angDist) * Math.cos(lat1), Math.cos(angDist) - Math.sin(lat1) * Math.sin(lat2));
    return { lat: toDeg(lat2), lon: (((toDeg(lon2) + 540) % 360) - 180) };
}
/** Linearly interpolate `steps` points (inclusive of both ends) between a and b. */
export function interpolatePoints(a, b, steps) {
    const points = [];
 ...
 */
export function computeLineOfSight(profile, observerHeight = 2, targetHeight = 2) {
    if (profile.length < 2) {
    ...
    const curvatureAndRefraction = (d) => ((1 - REFRACTION_COEFF) * d * d) / (2 * EARTH_RADIUS_M);
    let visible = true;
    let obstructedAt = null;
    const annotated = profile.map((p) => {
        ...
        const losHeight = observerElev + (targetElev - observerElev) * f - curvatureAndRefraction(d);
        return { ...p, losHeight, distance: d };
    });
    for (const p of annotated) {
        if (p.distance <= 0 || p.distance >= totalDistance)
            continue;
        if (p.elevation > p.losHeight) {
            visible = false;
            if (!obstructedAt)
                obstructedAt = { lat: p.lat, lon: p.lon };
            break;
        }
    }
...
export function computeSlopeAspect(grid, cellSizeM) {
...
export function sampleGrid(bbox, resolution) {
    const points = [];
