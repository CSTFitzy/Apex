...
const OPEN_METEO_ELEVATION_URL = process.env.OPEN_METEO_ELEVATION_API || 'https://api.open-meteo.com/v1/elevation';
...
export async function fetchElevations(points) {
    ...
    points.forEach((p, idx) => {
        ...
        if (elevationCache.has(key)) {
            results[idx] = { ...p, elevation: elevationCache.get(key) };
        }
        else {
            missingIdx.push(idx);
        }
...
export async function fetchElevation(point) {
